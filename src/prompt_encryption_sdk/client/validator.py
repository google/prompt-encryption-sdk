...
class AttestationValidator:
  ...
  def validate(
    ...
    self._verify_instance_key_binding(
        claims, instance_pub_bytes, expected_nonce=expected_nonce
    )
  ...
  def _verify_instance_key_binding(
    ...
    instance_key_hex = hashlib.sha256(pub_key_bytes).hexdigest()
    ...
    if instance_key_hex not in eat_nonce_list:
      raise exceptions.AttestationVerificationError(
          f"Instance Key binding failed. Key fingerprint {instance_key_hex}"
