...
class AttestationValidator:
  ...
  def validate(
    ...
    ecdsa_pub_bytes = response.instance_public_key.key_bytes
    if not ecdsa_pub_bytes:
      raise exceptions.AttestationVerificationError(
          "ECDSA public key is missing."
      )
    pqc_pub_bytes = response.pqc_public_key.serialized_public_keyset
    ...
    if not response.pqc_session_signature:
      raise exceptions.AttestationVerificationError(
    ...
    self._verify_session_signature_mldsa(
        response.pqc_public_key,
        signature=response.pqc_session_signature,
        payload=payload,
    )
