...
class AttestationValidator:
  ...
  def validate_proof(
    ...
    """Validates a role-neutral proof produced by either peer.

    The server uses this to check a confidential client's proof, so the checks
    below are deliberately symmetric: nothing here assumes the prover is the
    server.

    Args:
        proof: The proof to validate.
        tls_ekm: The Exported Keying Material from the TLS socket.
        expected_nonce: The fresh challenge nonce generated by the verifier.
        peer_role: Expected producer role included in the signed payload.
        mode: Negotiated attestation mode included in the signed payload.
    ...
    # TLS session. In mutual mode both peers bind to the same EKM, so
    # peer_role and mode are what stop a proof from being reflected back.
